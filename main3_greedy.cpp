...
//                    }
                    for (int i2 = 0; i2 < flights.size(); i2++) {
                        ...
                        cout << _tin[i] << "," << _tout[flights[i2]] << "," << _tin[flights[i2]] << "," << _tout[i] << endl;
                    ...
                    for (int j2 = 0; j2 < M; j2++) {
                        if (_conflictGatePosition[j][j2] == 1 || _conflictGatePosition[j2][j] == 1) {
                    ...
                    // 计算若安排进此机位所获取的价值
                    double valueJ = 3 * _distance[j]/(double)N + 3 * _distance[j] * X[i][j] * _numberOfFlight[i]/(double)(IloSum(_numberOfFlight)) - _temporaryMeasures[j]/(double)N - roadConflict/(double)N;
                    cout << "valueJ: " << valueJ << endl;
                    if (valueJ > value) {
                        bestPosition = j;
                        value = valueJ;
                    }
