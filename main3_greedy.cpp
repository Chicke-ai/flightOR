...
void solveByCplex()
{
    ...
    try {
        ...
        for (i = 0; i < N; i++)
        {
            ...
            int bestPosition = -1;
            for (j = 0; j < M; j++) {
                if ((_feasibleFlight[i][j] == 1) && (position[j] == -1 || ((_tin[i] - _tout[position[j]]) * (_tin[position[j]] - _tout[i]) <= 0))) {
                    int conflict = 0;
                    ...
                    int valueJ = 3 * _distance[j]/(double)N + 3 * X[i][j] * _numberOfFlight[i]/(double)(IloSum(_numberOfFlight)) - _temporaryMeasures[j]/(double)N - conflict/(double)N;
                    if (valueJ > value) {
                        bestPosition = j;
                        value = valueJ;
                    }
