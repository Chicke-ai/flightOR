...
////  FlightScheduling
////
////  Created by Jiang,Chao(AIP) on 18/2/28.
...
////                }
////            }
////            cout << "滑道冲突约束数量: " << 2 * total << endl;
//    
//            int total = 0;
//            for (i = 0; i < N_max - N_min - 1; i++)
//            {
//                IloNumExprArray xTemp3(env2, 2 * R);
//                IloNumExprArray xTemp4(env2, 2 * R);
//                int count1, count2;
//                for (k = 0; k < R; k++)
//                {
//                    cout << "滑道数量：" << R << endl;
//                    if (noLimit[k] == 1)
//                    {
//                        xTemp4[2*k] = IloNumExpr(env2);
//                        xTemp4[2*k + 1] = IloNumExpr(env2);
//                        for (j = 0; j < M; j++) {
//                            if (gatePositionToInRoad[j][k] == 1) {
//                                xTemp4[2*k] = xTemp4[2*k] + x[i][j];
//                            }
//                            if (gatePositionToOutRoad[j][k] == 1) {
//                                xTemp4[2*k + 1] = xTemp4[2*k + 1] + x[i][j];
//                            }
//                        }
//                        
//                        xTemp3[2*k] = IloNumExpr(env2);
//                        xTemp3[2*k + 1] = IloNumExpr(env2);
//                        count1 = 0;
//                        count2 = 0;
//                        for (j = 0; j < N_max - N_min; j++)
//                        {
//                            if (j == i)
//                            {
//                                continue;
...
//                                if (conflictFlightInIn[i][j] == 1 && gatePositionToInRoad[j2][k] == 1)
//                                {
//                                    // 精度修改
//                                    xTemp3[2*k] = xTemp3[2*k] + x[j][j2];
//                                    count1++;
//                                }
//                                if (conflictFlightOutOut[i][j] == 1 && gatePositionToOutRoad[j2][k] == 1)
//                                {
//                                    // 精度修改
//                                    xTemp3[2*k + 1] = xTemp3[2*k + 1] + x[j][j2];
//                                    count2++;
//                                }
//                                if (conflictFlightInOut[i][j] == 1 && gatePositionToOutRoad[j2][k] == 1)
//                                {
//                                    // 精度修改
//                                    xTemp3[2*k] = xTemp3[2*k] + x[j][j2];
//                                    count1++;
//                                }
//                                if (conflictFlightOutIn[i][j] == 1 && gatePositionToInRoad[j2][k] == 1)
//                                {
//                                    // 精度修改
//                                    xTemp3[2*k + 1] = xTemp3[2*k + 1] + x[j][j2];
//                                    count2++;
...
//                        if (count1 > 0) {
//                            total++;
//                            model.add(z2[i] >= xTemp4[2*k] + xTemp3[2*k]/(double)count1 - 1);
//                        }
//                        if (count2 > 0) {
//                            total++;
//                            model.add(z2[i] >= xTemp4[2*k + 1] + xTemp3[2*k + 1]/(double)count2 - 1);
//                        }
//                    }
//                }
//            }
//            cout << "滑道冲突约束数量: " << total << endl;
//
//            
...
//            //            cout << "================" << min << endl;
//            //            cout << "================" << max << endl;
//            map<int, vector<vector<int>>> p_conflict_map = map<int, vector<vector<int>>>();
//            for (j = 0; j < M; j++)
//            {
//                cout << "机位 " << j << endl;
//                vector<vector<int>> p_t_f_conflict = vector<vector<int>>();
//                //                p_conflict_map.insert(make_pair(j, p_t_f_conflict));
//                // 一个机位下的所有时刻的冲突航班集合
//                for (int i2 = 0; i2 < N_max - N_min; i2++)
//                {
//                    //                    cout << "时间 " << t << endl;
//                    //                    cout << "当前已有冲突集合 {" << endl;
//                    //                    for (vector<vector<int>>::const_iterator iter = p_t_f_conflict.cbegin(); iter != p_t_f_conflict.cend(); iter++)
//                    //                    {
//                    //                        vector<int> exit_conflict = *iter;
//                    //                        cout << "[" << endl;
//                    //                        for (i = 0; i < exit_conflict.size(); i++) {
//                    //                            cout << exit_conflict[i] << "," << endl;
//                    //                        }
//                    //                        cout << "]" << endl;
//                    //                    }
//                    //                    cout << "}" << endl;
//                    
//                    vector<int> t_f_conflict = vector<int>();
//                    for (i = 0; i < N_max - N_min; i++) {
//                        if ((feasibleFlight[i][j] == 1) && (tin[i2] + 1 > tin[i]) && (tin[i2] + 1 < tout[i]))
//                        {
//                            //                            cout << "航班 " << i << endl;
//                            t_f_conflict.push_back(i);
//                        }
//                    }
//                    // 判断是否符合加入条件，如果有覆盖则只保留最大团
//                    if (t_f_conflict.size() > 1) {
//                        bool flag = true;
//                        for (vector<vector<int>>::const_iterator iter = p_t_f_conflict.cbegin(); iter != p_t_f_conflict.cend(); )
//                        {
//                            if (isSubset(*iter, t_f_conflict))
//                            {
//                                // 集合里面有更大的团，不要再加入
//                                //                                cout << "集合里面有更大的团，不需再加入" << endl;
//                                flag = false;
//                                break;
//                            }
//                            if (isSubset(t_f_conflict, *iter))
//                            {
//                                //                                cout << "集合里面有子集，删除集合数据" << endl;
//                                iter = p_t_f_conflict.erase(iter);
//                            } else {
//                                iter++;
//                            }
//                        }
//                        if (flag) {
//                            //                            cout << "新增冲突" << endl;
//                            p_t_f_conflict.push_back(t_f_conflict);
//                        }
//                    }
//                }
//                // 转为约束条件
//                IloNumExprArray xTemp2(env2, p_t_f_conflict.size());
//                for (int c = 0; c < p_t_f_conflict.size(); c++) {
//                    xTemp2[c] = IloNumExpr(env2);
//                    vector<int> exist_conflict = p_t_f_conflict[c];
//                    //                    cout << "机位" << j << " 某时刻冲突航班数量：" << exist_conflict.size() << endl;
//                    for (i = 0; i < exist_conflict.size(); i++)
//                    {
//                        xTemp2[c] = xTemp2[c] + x[exist_conflict[i]][j];
//                    }
//                    model.add(xTemp2[c] <= 1);
//                }
//            }
//            
//            clock_t time1 = clock();
...
//            //                }
//            //            }
//            //            cout << "优化航班所使用滑道的约束数量: " << total << endl;
//            
//            // 约束条件4 冲突机位不能同时使用
...
//            
//            clock_t time2 = clock();
//            cout << "CPLEX求解用时2222 : " << (time2-time1) / CLOCKS_PER_SEC << " 秒" << endl;	//显示花费时间
//            
