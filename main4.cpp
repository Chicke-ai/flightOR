...
//        cout << endl;
        ...
        while (runTimes > 0)
        {
            cout << "run time: " << runTimes << endl;
            IloEnv env2;
            //            N_max += N_once_1;
            vector<int> randomPositions = vector<int>();
            srand((unsigned)time(NULL));
            while (randomPositions.size() < 20) {
                cout << "random positions: " << randomPositions.size() << endl;
                int index = 0 + rand() % (M + 0 - 1);
                cout << "index: " << index << endl;
                bool exist = false;
                for (k = 0; k < randomPositions.size(); k++) {
                    if (randomPositions[k] == index) {
                        exist = true;
                        break;
                    }
                    continue;
                ...
                if (!exist) {
                    cout << "add index: " << endl;
                    randomPositions.push_back(index);
                }
            }
            ...
            for (i = 0; i < N_max - N_min; i++)
            {
                ...
                for (j = 0; j < M; j++) {
                    bool drop = false;
                    for (k = 0; k < randomPositions.size(); k++) {
                        if (randomPositions[k] == j) {
                            drop = true;
                            break;
                        }
                    }
                    if (!drop) {
                        model.add(x[i][j] == X[i][j]);
                    }
                }
            }
